...
function createRecipeForVerifyDiv() {
    ...
    getData('/api/notVerifiedRecipes').then((data) => {
        ...
        <div class="recipe-layout__recipe-data-layout">
            <div class="recipe-layout__recipe-id">${data[0][0].Recipe_ID}</div>
            <img src="${data[0][0].Picture_data}" class="recipe-layout__recipe-image" />
            <div class="recipe-layout__recipe-title">${data[0][0].Title}</div>
        </div>
        <div class="recipe-layout__button-layout">
            <button type="button" id="${data[0][0].Recipe_ID}" onclick="viewRecipe(this.id)" class="recipe-layout__watch-button">
                <img class="recipe-layout__icon" src="../images/adminPage_Image/icon3.svg" />
                <div class="recipe-layout__text">Megtekintés</div>
            </button>
            <button type="button" id="${data[0][0].Recipe_ID}" onclick="verifyRecipe(this.id)" class="recipe-layout__ok-button">
                <img class="recipe-layout__icon" src="../images/adminPage_Image/icon4.svg" />
                <div class="recipe-layout__text">Jóváhagyás</div>
            </button>
            <button type="button" id="${data[0][0].Recipe_ID}" onclick="deleteRecipe(this.id)" class="recipe-layout__delete-button">
